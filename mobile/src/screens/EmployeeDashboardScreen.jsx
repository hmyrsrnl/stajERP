...
export default function EmployeeDashboardScreen({ navigation }) {
  ...
  const fetchData = async () => {
    try {
      ...
      if (empRes.data) {
        setEmployeeInfo(empRes.data.data || empRes.data);
      }
      ...
      try {
        ...
        if (exams.length > 0) {
          const typesMap = {};
          exams.forEach(item => {
            const type = item.exam_type || item.muayene_turu || 'Genel';
      ...
      try {
        ...
        certs.forEach(c => {
          const expDateStr = c.expiry_date || c.gecerlilik_tarihi;
          const isExpired = expDateStr && new Date(expDateStr) < today;
          const isPassive = (c.status || c.durum || '').toLowerCase() === 'pasif';
        ...
        console.log("Sertifikalar çekilemedi:", e);
      }
  ...
  return (
    ...
    >
      <Header
        title={employeeInfo ? `Hoş Geldiniz, ${employeeInfo.first_name || employeeInfo.Ad} ${employeeInfo.last_name || employeeInfo.Soyad}` : 'Çalışan Paneli'}
      ...
      <View style={styles.mainContent}>
        ...
        <View style={styles.cardWrapper}>
          <Text style={styles.cardTitle}>Sağlık Sertifikası Durumum </Text>
